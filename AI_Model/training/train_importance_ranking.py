...
CSV_PATH = PROJECT_ROOT / "data" / "importance_ranking_1000.csv"
SAVE_DIR = PROJECT_ROOT / "models" / "importance_ranker"
OUT_DIR = PROJECT_ROOT / "outputs" / "importance_ranker"
...
def clean_dataframe(df):
    ...
    df = df[
        ...
        (df["importance_label"].str.len() > 1) &
        (df["importance_score"].str.len() > 0) &
        (df["importance_reason"].str.len() > 5)
...
def build_target(row) -> str:
    data = {
        "importance_label": str(row["importance_label"]).strip(),
        "importance_score": str(row["importance_score"]).strip(),
        "importance_reason": str(row["importance_reason"]).strip(),
    }
...
def main():
    ...
    df = load_csv_dataframe(
        CSV_PATH,
        required_cols=["chunk_text", "importance_label", "importance_score", "importance_reason"],
    )
